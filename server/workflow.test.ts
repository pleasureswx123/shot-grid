...
import { applyVersionStatusEffects, assertTaskStatusTransition, assertVersionStatusTransition, deriveEntityStatus, isEntityLockedForNonAdmin } from './workflow';

const version = { id: 'version-1', task_id: 'task-1', entity_type: 'shot' as const, entity_id: 'shot-1' };

const createClient = (entityTasks: Array<{ pipeline_stage: string; status: string; has_final?: boolean }> = [], entityStatus = '未开始') => {
  const queries: Array<{ sql: string; params?: unknown[] }> = [];
  return {
    queries,
    async query(sql: string, params?: unknown[]) {
      queries.push({ sql, params });
      if (sql.includes('SELECT status FROM shots') || sql.includes('SELECT status FROM assets')) return { rows: [{ status: entityStatus }], rowCount: 1 };
...
test('审核退回会让任务进入修改中并让镜头回到制作中', async () => {
  ...
  assert.deepEqual(client.queries.map(query => query.params), [
    ['修改中', 'task-1'], ['shot-1'], ['shot', 'shot-1'], ['视频生成', '制作中', 'shot-1'],
  ]);
});
...
test('明确锁定优先于全部任务完成且非管理员锁定检查生效', async () => {
  const client = createClient([
...
test('多任务派生状态遵循锁定、审核、制作、完成的优先级', () => {
  const tasks = (...statuses: Array<'未开始' | '制作中' | '待审核' | '修改中' | '已完成' | '已阻塞'>) => statuses.map(status => ({ status }));
  assert.equal(deriveEntityStatus('shot', tasks('待审核', '修改中', '已完成'), false), '审核中');
  assert.equal(deriveEntityStatus('asset', tasks('未开始', '已阻塞', '已完成'), false), '制作中');
  assert.equal(deriveEntityStatus('shot', tasks('已完成', '已完成'), false), '已完成');
  assert.equal(deriveEntityStatus('asset', tasks('已完成', '已完成'), false), '已定稿');
  assert.equal(deriveEntityStatus('shot', tasks('待审核', '制作中'), true), '已锁定');
});
